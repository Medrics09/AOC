...
class Two extends AbstractTask {
    ...
    protected function buildGift(array $components): int
    {
        ...
        $totalSize = $this->dirs['/'];
        $needToDeleteSize = $totalSize - (self::DISK_SIZE - self::UPDATE_SIZE);

        $possibleDirs = array_filter($this->dirs, function (int $dirSize) use ($needToDeleteSize) {
            return $dirSize >= $needToDeleteSize;
        });
